...
mycursor.execute("CREATE TABLE IF NOT EXISTS rfid_log (rfid VARCHAR(255), timestamp VARCHAR(255))")
...
# Conseguir tiempo actual del sistema
    kernel_time = time.strftime('%Y-%m-%d %H:%M:%S')

# Manda el tiempo actual del sistema a la base de datos
    sql = "INSERT INTO rfid_log (rfid, timestamp) VALUES (%s, %s)"
    val = (rfid, kernel_time)
    mycursor.execute(sql, val)
    mydb.commit()

# Notifica de que todo ha ido bien
